...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 17 14:41:20 2019
...
class activityClassifier_HMM_db2:
...
#            Q[:,i] = sin_wavelet(data,aa,normalize=True)
            if pc in self.square:
                trans, dil = db2_wavelet(data**2,aa, ret_scale=True)
                Q[dil-1:,i] = trans 
            else:
                trans, dil = db2_wavelet(data,aa, ret_scale=True)
                Q[dil-1:,i] = trans 
        return np.log(np.abs(Q))
...
#                print(i)
    ...
    #            Q[:,i] = sin_wavelet(data,aa,normalize=True)
                if j in self.square:
                    trans, dil = db2_wavelet(data[:,j]**2,aa, ret_scale=True)
                    Q[dil-1:,j*self.a.size+i] = trans
                else:
                    trans, dil = db2_wavelet(data[:,j],aa, ret_scale=True)
                    Q[dil-1:,j*self.a.size+i] = trans
        return np.log(np.abs(Q))
